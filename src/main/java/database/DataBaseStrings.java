...
public class DataBaseStrings {
	...
	public static final String CREATE_SURVEY_TABLE = "CREATE TABLE IF NOT EXISTS SurveysTable (surveyId INTEGER PRIMARY KEY AUTO_INCREMENT,"
			...
			+ "peopleVoted INTEGER, answerOptions INTEGER, inlineMsgId VARCHAR (200), inlineQueryResultArticleId VARCHAR (200),"
			+ "surveyText VARCHAR (500));";
	//Sentencia de insert en la tabla de las encuestas COMPROBADA Y OK.
	public static final String INSERT_SURVEY = "INSERT INTO SurveysTable (userId, question, answers, score, peopleVoted, answerOptions, inlineMsgId, inlineQueryResultArticleId, surveyText) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);";
	//Sentencia de lectura de la tabla de encuestas COMPROBADA Y OK! EN MYSQL SE guarda un "." en vez de una ",".
	public static final String READ_SURVEY = "SELECT question, answers, score, peopleVoted, answerOptions, inlineMsgId, inlineQueryResultArticleId, surveyText FROM SurveysTable WHERE userId = ?;";
	...
	public static final String UPDATE_SURVEY = "UPDATE SurveysTable SET question = ?, answers = ?, score = ?, peopleVoted = ?, answerOptions = ?, surveyText = ? WHERE userId = ? AND inlineQueryResultArticleId = ?;";
